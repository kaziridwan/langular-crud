...
<script type="text/javascript">
    ...
    invoiceApp.controller('InvoiceController', ['$scope','$http', function($scope,$http){
            {{'$scope.customer_id = '. $invoice->customer->id . ';'}}
            $scope.date = '{{$invoice->date}}';
            $scope.due_date = "{{$invoice->due_date}}";
            $scope.memo = "{{$invoice->memo}}";



            $http.get('../../products').success(function(data){
                $scope.products = data;
                $http.get('../../invoices/{{$invoice->id}}/invoice_items').success(function(data){
                    $scope.invoice_items = data;
                    angular.forEach($scope.invoice_items,function(item,index){
            ...
            $scope.save = function(draft) {
                ...
                $http.put('../{{$invoice->id}}',invoice).success(function(response){
                    console.log(response);
                    window.location = '<?php echo URL::to('/invoices/'.$invoice->id);?>';
